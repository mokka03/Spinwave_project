...
'''Directories'''
basedir = 'Ms_change_125670Aperm_to_145670Aperm/'
...
for Ms_ in range(125670,146670,1000):
    '''Geometry, sources, probes, model definitions'''
    # geom = spintorch.WaveGeometryFreeForm((nx, ny), dx, dy, dz, B0, B1, Ms)
    geom = spintorch.WaveGeometryMs((nx, ny), dx, dy, dz, Ms, B0, Ms_)
    ...
    with torch.no_grad():
        ...
        plt.gcf().savefig(plotdir+'field_snapshot_%dAperm.png' % (Ms_))

        u = spintorch.utils.normalize_power(model(INPUTS).sum(dim=1))    
        spintorch.plot.plot_output(u[0,], OUTPUTS[0]+1, Ms_, "Aperm", plotdir)
        outs = u[0,].detach().cpu().numpy()
        out = outs[int(Np/2)]
        out_list.append(out)
        Ms_values.append(Ms_)
    stat_cuda('befor del')
    del u_field
...
Ms_out = np.zeros((2,Ms_array.shape[0]))
Ms_out[0,] = Ms_array
Ms_out[1,] = out_array

savemat("Ms_out.mat", {"Ms_out": Ms_out})