...
'''Directories'''
basedir = 'focus_binary_lens_YIGdubs2_50umx50um_alpha/'
...
with torch.no_grad():
    ...
    savemat("Msat50alpha.mat", {"Msat": Msat.to(torch.device("cpu")).numpy().transpose()})
    m = u_field[0,4999,]
    savemat("m50alpha.mat", {"m": m.to(torch.device("cpu")).numpy().transpose()})
    print('--------------------------------------------------')
