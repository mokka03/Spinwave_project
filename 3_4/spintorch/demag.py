...
def demag_tensor_fft(nx, ny, nz, dx, dy, dz, dev, z_off=0):
    ...
    # Kxx, Kyy, Kzz, Kxy = demag_tensor(nx, ny, nz, dx, dy, dz, z_off)

    Kx_fft = tr.rfft(tr.tensor(stack((Kxx[:,:,:], Kxy[:,:,:], Kxz[:,:,:]),0),
                                     device=dev, dtype=tr.float32).unsqueeze(0), 3, onesided=False)
    Ky_fft = tr.rfft(tr.tensor(stack((Kxy[:,:,:], Kyy[:,:,:], Kyz[:,:,:]),0),
                                     device=dev, dtype=tr.float32).unsqueeze(0), 3, onesided=False)
    Kz_fft = tr.rfft(tr.tensor(stack((Kxz[:,:,:], Kyz[:,:,:], Kzz[:,:,:]),0),
                                     device=dev, dtype=tr.float32).unsqueeze(0), 3, onesided=False)
...
def demag_tensor_fft_2D(nx, ny, dx, dy, dz, dev):
    ...
    Kxx_fft = tr.rfft(tr.tensor(Kxx, device=dev, dtype=tr.float32
                                ).unsqueeze(0), 2, onesided=False)
    Kyy_fft = tr.rfft(tr.tensor(Kyy, device=dev, dtype=tr.float32
                                ).unsqueeze(0), 2, onesided=False)
    Kzz_fft = tr.rfft(tr.tensor(Kzz, device=dev, dtype=tr.float32
                                ).unsqueeze(0), 2, onesided=False)
    Kxy_fft = tr.rfft(tr.tensor(Kxy, device=dev, dtype=tr.float32
                                ).unsqueeze(0), 2, onesided=False)
    Kxz_fft = tr.rfft(tr.tensor(Kxz, device=dev, dtype=tr.float32
                                ).unsqueeze(0), 2, onesided=False)
    Kyz_fft = tr.rfft(tr.tensor(Kyz, device=dev, dtype=tr.float32
                                ).unsqueeze(0), 2, onesided=False)
...
if __name__ == "__main__":                                                          # Ez egy próba rész a demag.py tesztelésére
    ...
    # Kxx, Kyy, Kzz, Kxy = demag_tensor_2D(nx, ny, dx)
    ...
    KxM = complex_multiply(tr.stack([Kxx_fft, Kxy_fft],1), m_fft[:,0:2,])
