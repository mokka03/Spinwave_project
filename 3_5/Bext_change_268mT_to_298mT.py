...
timesteps = 5556 # number of timesteps for wave propagation

nx = 500        # size x
ny = 500        # size y

'''Directories'''
basedir = 'Bext_change_268mT_to_298mT/'
...
for B_mT in range(268,299,1):
    ...
    with torch.no_grad():
        ...
        plt.gcf().savefig(plotdir+'field_snapshot_%dmT.png' % (B_mT))

        u = spintorch.utils.normalize_power(model(INPUTS).sum(dim=1))    
        spintorch.plot.plot_output(u[0,], OUTPUTS[0]+1, B_mT, "mT", plotdir)
        outs = u[0,].detach().cpu().numpy()
        out = outs[int(Np/2)]
        out_list.append(out)
        Bext_values.append(B0)
...
Bext_array = np.array(Bext_values)
Bext_out = np.zeros((2,Bext_array.shape[0]))
Bext_out[0,] = Bext_array
Bext_out[1,] = out_array

savemat("Bext_out.mat", {"Bext_out": Bext_out})